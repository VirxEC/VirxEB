...
class Prediction(Thread):
    ...
    def time_to_ball(self, car):
        ...
        for ball_slice in self.agent.ball_prediction_struct.slices[12::6]:
        ...
        while self.online:
            try:
                ...
                if len(self.agent.foes) > 0:
                    ...
                    if len(foe_distances) > 0:
                        if self.agent.odd_tick == 0:
                            self.agent.predictions['enemy_time_to_ball'] = min(tuple(self.time_to_ball(foe) if not foe.demolished else math.inf for foe in self.agent.foes))
                ...
                if self.agent.odd_tick == 0:
                    if self.agent.goalie:
                        ...
                        if ball_loc_y < 2560:
                            ...
                            self_time_to_ball = self.time_to_ball(self.agent.me)
                            team_time_to_ball = min(tuple(self.time_to_ball(teammate) if not teammate.demolished else math.inf for teammate in self.agent.friends))
                            if ball_loc_y < -1280 and self_time_to_ball < team_time_to_ball and self.agent.predictions['self_from_goal'] != self.agent.predictions["team_from_goal"][0]:
                                ...
                                self.agent.playstyle = self.agent.playstyles.Defensive if self.agent.gravity.z < -550 and len_friends > 1 or (len_friends == 1 and (self.agent.predictions['was_down'] or abs(self.agent.game.friend_score - self.agent.game.foe_score) > 1)) else self.agent.playstyles.Neutral
                            else:
                ...
                if not can_shoot and self.agent.can_shoot is None:
                    self.agent.can_shoot = self.agent.time - 2.9
